...
int main(int argc, char** argv) {
    ...
    while(std::getline(input_file, trace)) {
        ...
        if (activity == "CPU") {
	        ...
	        execution += std::to_string(now_ms) + ", " + std::to_string(ISR_START_ADRS) + ", find vector " + std::to_string(duration_intr) + " in memory position " + std::to_string(mem_pos) + "\n";
	        now_ms += ISR_START_ADRS;

	        // get ISR address from vector table
	        execution += std::to_string(now_ms) + ", " + std::to_string(GET_ISR_ADRS) + ", obtain ISR address (" + vectors.at(duration_intr) + ")\n";
	        now_ms += GET_ISR_ADRS;

	        // execute ISR body
            execution += std::to_string(now_ms) + ", " + std::to_string(delays.at(duration_intr)) + ", call device driver\n";
            now_ms += delays.at(duration_intr);
	        device_finish[duration_intr] = now_ms;	
            ...
            // execute ISR body
            execution += std::to_string(now_ms) + ", " + std::to_string(delays.at(duration_intr)) + ", end of I/O " + std::to_string(duration_intr) + ": interrupt\n";
            now_ms += delays.at(duration_intr);
